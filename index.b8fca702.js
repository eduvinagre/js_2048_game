...
const gameCells = Array.from(gameField.querySelectorAll(".field-row")).map((row)=>Array.from(row.children));
...
class Game {
    ...
    static get deafultInitialState() {
        return new Array(Game.deafultSize).fill(null).map((el)=>new Array(Game.deafultSize).fill(0));
    }
    // current game status
    #status = "idle";
    // current score
